...
It differs from the normal ICAROS map because here we use
a different number of XY bins to fit the lifetime. We also use S2q 
rather than S2e.
"""
import os
import logging
...
from krcal.map_builder.map_builder_functions  import calculate_map_sipm_diffbins
...
from krcal.map_builder.map_builder_functions  import check_failed_fits_diffbins
from krcal.map_builder.map_builder_functions  import regularize_map
from krcal.map_builder.map_builder_functions  import remove_peripheral_diffbins
...
def MakeMap(sel_dst, q_range, lt_range, output_maps_folder, name, dst, sel_krband, map_file_out):

    ### Make Map
    print(sel_dst.event.nunique(), 'events')
    number_of_bins = 50
    number_of_bins_lt = 20
    print('Number of XY bins, lt XY bins: ', number_of_bins, number_of_bins_lt)
    ...
    maps = calculate_map_sipm_diffbins(dst        = sel_dst  ,
                        XYbins     = (number_of_bins         ,
                                      number_of_bins)        ,
                        XYbins_lt  = (number_of_bins_lt      ,
                                     number_of_bins_lt)      ,
    ...
    check_failed_fits_diffbins(maps      = maps          ,
                    maxFailed = maxFailed     ,
                    nbins     = number_of_bins,
                    nbins_lt  = number_of_bins_lt,
                    rmax      = r_max         ,
                    rfid      = r_max         )
    ...
    regularized_maps = remove_peripheral_diffbins(map   = regularized_maps,
                                        nbins = number_of_bins  ,
                                        nbins_lt = number_of_bins_lt,
                                        rmax  = r_max           ,
                                        rfid  = r_max           )
...
if __name__ == '__main__':
    ...
    lt_range = (0, 20000)
    map_file_out     = os.path.join(output_maps_folder, f'map_sipm_{run_number}_{name}_TESTDIFFBINS.h5')
    MakeMap(sel_dst, q_range, lt_range, output_maps_folder, name, dst, sel_krband, map_file_out)

    this_map = read_maps(map_file_out)
    q_range = (600,850)
