...
plt.savefig(outputdir+'corrections_pmt.png')
plt.close()

e_range = (10000, 13500) #(400,1000)
z_range = (10, 550)
fig = plt.figure(figsize=(14,8))
plt.subplot(1, 2, 1)

nevt = h2(sel_dst.Z, sel_dst.S2e*corr_tot, 30, 70, z_range, e_range, profile=True)
plt.xlabel('Z (mm)');
plt.ylabel('E (pes)');
plt.title('E vs Z');
...
plt.ylabel('Entries');
plt.title('E corr');
plt.savefig(outputdir+'corrections_2_pmt.png')
plt.close()

fc = fit_energy(sel_dst.S2e*corr_tot, nbins=100, range=e_range)
plot_fit_energy(fc)
print_fit_energy(fc)
plt.savefig(outputdir+'eres_pmt.png')
plt.close()
...
plt.savefig(outputdir+'eres_by_r_pmt.png')
plt.close()
...
fc = fit_lifetime(sel_dst.Z, sel_dst.S2e*corr_geo, 50, 50, (0,zcut), e_range)
plot_fit_lifetime(fc)
plt.savefig(outputdir+'lt_test_pmt.png')
plt.close()